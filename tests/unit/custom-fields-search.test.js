...
describe('Custom Fields Search Integration', () => {
	...
	describe('Dynamic search key generation', () => {
		...
		it('should generate search keys for all fields', () => {
			...
			expect(keys).toContainEqual({ name: 'customFields.component', weight: 1.5 });
			expect(keys).toContainEqual({ name: 'customFields.taskType', weight: 1.6 });
			expect(keys).toContainEqual({ name: 'customFields.assignee', weight: 1.2 });
		});
		...
		it('should handle tasks with no custom fields', () => {
			...
			expect(keys.every(key => !key.name.includes('customFields'))).toBe(true);
		});
...
export { generateSearchKeys, searchTasks, getCustomFieldNames, getWeightForField };