...
export function migrateLegacyFormatWithCustomFields(legacyData) {
	...
	const tasksWithCustomFields = ensureTasksBackwardCompatibility(legacyData.tasks);
...
export function ensureDataBackwardCompatibility(data, isLegacyFormat = false) {
	...
	for (const tagName in data) {
		if (Object.prototype.hasOwnProperty.call(data, tagName) && 
			...
			processedData[tagName] = ensureTagDataBackwardCompatibility(data[tagName]);
		} else {
 ...
 */
export function createBackwardCompatibleResult(tagData, resolvedTag, originalTaggedData) {
