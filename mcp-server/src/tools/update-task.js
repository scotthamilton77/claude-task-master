...
/**
 * Create base schema for update-task parameters (without custom fields)
 */
function createBaseUpdateTaskSchema() {
	return z.object({
		id: z
...
export function registerUpdateTaskTool(server) {
	server.addTool({
		name: 'update_task',
		description:
			'Updates a single task by ID with new information, context, or custom fields. Custom fields are dynamically configured per project via .taskmaster/custom-fields.json',
		parameters: createBaseUpdateTaskSchema().passthrough(), // Use base schema with passthrough
